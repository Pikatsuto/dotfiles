    ...
    boot = {
      ...
      extraModprobeConfig = let
        ...
        in lib.optionalString ("${vendors}" != "") ''
          options vfio-pci ids=${vendors} disable_vga=1
        '';
      ...
      let
        ...
        pciesXml = optionalConcatFor {
          ...
          return = (pcie: optionalConcatFor {
            ...
            return = (function: let
              bar = if function.fix.romBar
                then "on"
                else "off";
              pcieId =
                "${pcie.lines.bus}:${pcie.lines.slot}.${function.function}";
              rom = ifElse (function.fix.rom && !function.fix.voidRom && (isVga pcieId)) ''
                <rom bar="${bar}" file="/var/lib/libvirt/roms/pcie-${pcieId}.rom"/>
              '' lib.optionalString (function.fix.voidRom && !function.fix.rom && (isVga pcieId)) ''
                <rom bar="${bar}" file="/var/lib/libvirt/roms/pcie-${pcieId}-void.rom"/>
              '';
            in ''
              <hostdev
                mode='subsystem'
        ...
        generateVoidRoms = optionalConcatFor {
          condition = vm.passthrough.enable;
          array = vm.passthrough.pcies;
          return = (pcie: optionalConcatFor {
            condition = ! pcie.disk;
            array = pcie.lines.functions;
            return = (function: lib.optionalString (function.fix.voidRom) (let
              pcieId = "${pcie.lines.bus}:${pcie.lines.slot}.${function.function}";
            in lib.optionalString (isVga pcieId) ''
              if [ ! -f "/var/lib/libvirt/roms/pcie-${pcieId}-void.rom" ]; then
                dd if=/dev/zero \
                  of=/var/lib/libvirt/roms/pcie-${pcieId}-void.rom \
                  bs=1M count=1
              fi
            ''));
          });
        ...
        smartAccessMemory = (lib.optionalString (vm.passthrough.smartAccessMemory) ''
          <qemu:commandline>
